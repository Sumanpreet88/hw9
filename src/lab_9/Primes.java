...
public class Primes
{
    ...
    public static void main(String args[])
    {
        ...
        ListInterface<Integer> composites=new AList<Integer>();// COMPLETE THE MAIN
        ...
        while(candidates.getLength()!=0)//LOOP TILL list is not empty
        {//first entry will be a prime number always
        	int prime=candidates.getEntry(1);
        	candidates.remove(1);  //remove it and add it to primes list
        	primes.add(prime);
            //now this function will remove all the multiples of prime which are less than max 
        	//from candidates list and add them to composites list
        	getComposites(candidates,composites,prime);
        }
        System.out.println("prime numbers less than or equal to "+max+" are");
